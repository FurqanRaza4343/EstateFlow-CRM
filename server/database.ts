...
function getSeedLeads(): Lead[] {
  return [
    // org-estateflow-1
    { id: 'lead-1', organizationId: 'org-estateflow-1', fullName: 'Zain Malik', phone: '+92 300 1234567', email: 'zain.malik@example.com', source: '36 Acre', propertyType: 'Apartment', budgetMin: 15000000, budgetMax: 35000000, preferredLocation: 'DHA Phase 6, Lahore, Pakistan', status: 'New', temperature: 'Hot', assignedAgentId: 'user-agent-1', notes: 'Interested in luxury residential phase. Needs immediate dial-back.', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
    { id: 'lead-2', organizationId: 'org-estateflow-1', fullName: 'Ayesha Khan', phone: '+92 321 9876543', email: 'ayesha.khan@gmail.com', source: 'MagicBricks', propertyType: 'Apartment', budgetMin: 12000000, budgetMax: 20000000, preferredLocation: 'Clifton Block 5, Karachi, Pakistan', status: 'Contacted', temperature: 'Warm', assignedAgentId: 'user-agent-2', notes: 'Wants sea-view apartment. Requested pricing brochure.', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
    { id: 'lead-3', organizationId: 'org-estateflow-1', fullName: 'Sarah Jenkins', phone: '+1 (555) 019-2834', email: 'sarah.jenkins@gmail.com', source: 'Facebook Ads', propertyType: 'Villa', budgetMin: 50000000, budgetMax: 80000000, preferredLocation: 'Downtown Manhattan, New York, USA', status: 'Site Visit Scheduled', temperature: 'Hot', assignedAgentId: 'user-agent-1', notes: 'Wants high-rise luxury flat options. Demands supreme privacy.', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
    { id: 'lead-4', organizationId: 'org-estateflow-1', fullName: 'Bilal Ahmed', phone: '+92 333 5551234', email: 'bilal.ahmed@example.com', source: 'Website', propertyType: 'Commercial', budgetMin: 30000000, budgetMax: 50000000, preferredLocation: 'Sector F-7, Islamabad, Pakistan', status: 'Interested', temperature: 'Warm', assignedAgentId: 'user-agent-2', notes: 'Looking for premium commercial setup of 1500+ sq ft.', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
    { id: 'lead-5', organizationId: 'org-estateflow-1', fullName: 'Tariq Al-Mansoor', phone: '+971 50 123 4567', email: 'tariq.almansoor@gmail.com', source: 'Referral', propertyType: 'Apartment', budgetMin: 30000000, budgetMax: 40000000, preferredLocation: 'Dubai Marina, Dubai, UAE', status: 'Negotiation', temperature: 'Hot', assignedAgentId: 'user-agent-1', notes: 'NRI client in Dubai. In final stages of booking sea-view unit.', isHot: true, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
...
function getSeedFollowUps(): FollowUp[] {
  return [
    { id: 'fup-1', organizationId: 'org-estateflow-1', leadId: 'lead-1', agentId: 'user-agent-1', datetime: new Date(Date.now() + 4 * 3600 * 1000).toISOString(), notes: 'Call Zain back for Lahore DHA Phase 6 high-rise floor confirmation.', completed: false, type: 'Call' },
    { id: 'fup-2', organizationId: 'org-estateflow-1', leadId: 'lead-3', agentId: 'user-agent-1', datetime: new Date(Date.now() + 25 * 3600 * 1000).toISOString(), notes: 'Meet Sarah Jenkins for downtown NY penthouse visit accompaniment.', completed: false, type: 'Site Visit' },
...
function getSeedActivities(): Activity[] {
  return [
    { id: 'act-1', organizationId: 'org-estateflow-1', leadId: 'lead-1', userId: 'user-agent-1', type: 'System', title: 'Lead Created', description: 'Zain Malik entered via webhook source 36 Acre. Auto-assigned to Sales Agent Arjun Kumar.', timestamp: new Date(Date.now() - 2 * 3600 * 1000).toISOString() },
...
function getSeedCalls(): CallLog[] {
  return [
    { id: 'call-1', organizationId: 'org-estateflow-1', leadId: 'lead-1', agentId: 'user-agent-1', status: 'Completed', duration: 112, startedAt: new Date(Date.now() - 1.8 * 3600 * 1000).toISOString(), endedAt: new Date(Date.now() - 1.78 * 3600 * 1000).toISOString(), outcome: 'Positive response. Client agreed to review DHA brochure.' },
  ...
  public static initialize(): DBStore {
    ...
    try {
      ...
      if (fs.existsSync(DB_FILE)) {
        ...
        // Migration for multi-currency and multi-language support
        this.store.organizations = this.store.organizations.map((org, index) => {
          const seed = seedOrganizations.find(s => s.id === org.id) || seedOrganizations[index] || seedOrganizations[0];
          return {
            ...org,
            languagePreference: org.languagePreference || seed.languagePreference || 'en',
            currencyPreference: org.currencyPreference || seed.currencyPreference || 'USD',
            propertyUnitSystem: org.propertyUnitSystem || seed.propertyUnitSystem || 'global',
          };
        });
        if (this.store.organization) {
          const matchOrg = this.store.organizations.find(o => o.id === this.store!.organization.id) || this.store.organizations[0];
          this.store.organization = matchOrg;
        }
        ...
        notifications: [
          {
            ...
            description: 'Zain Malik assigned. Take action immediate.',
            type: 'LeadAssigned',
            isRead: false,
            createdAt: new Date().toISOString()
          }
