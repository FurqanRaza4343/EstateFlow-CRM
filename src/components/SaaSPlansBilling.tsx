...
export function convertAndFormatPrice(usdAmount: number, currency: 'USD' | 'AED' | 'PKR' = 'USD'): string {
  if (usdAmount === 0) {
    if (currency === 'PKR') return 'Rs. 0';
    if (currency === 'AED') return '0 AED';
    return '$0';
  }

  const multiplier = exchangeRates[currency] || 1;
  ...
  if (currency === 'PKR') {
    ...
    return `Rs. ${new Intl.NumberFormat('en-PK').format(rounded)}`;
  }
...
export default function SaaSPlansBilling({
  ...
  const handleProcessPayment = (e: React.FormEvent) => {
    ...
    setProcessLogs(['Initiating payment gateway...', 'Connecting to secure checkout...']);
    ...
    setTimeout(async () => {
      ...
      const priceKey = billingPeriod === 'monthly' ? `${selectedPlanId}_monthly` : `${selectedPlanId}_yearly`;
      const priceMap: Record<string, string> = {
        'Free_monthly': 'price_free',
        'Plus_monthly': 'price_plus_monthly', 'Plus_yearly': 'price_plus_yearly',
        'Pro_monthly': 'price_pro_monthly', 'Pro_yearly': 'price_pro_yearly',
        'Business_monthly': 'price_business_monthly', 'Business_yearly': 'price_business_yearly',
        'Platinum_monthly': 'price_platinum_monthly', 'Platinum_yearly': 'price_platinum_yearly',
      };
      ...
      try {
        ...
        if (checkoutData.simulated || checkoutData.url?.includes('sim_checkout')) {
          ...
          setProcessLogs(prev => [...prev, '⚠ Simulation mode: using mock checkout (no Stripe key configured)']);

          // Simulate decline for card ending in 4000
          if (paymentMethodSelection === 'card' && formCardNumber.endsWith('4000')) {
            setCheckoutStep('failed');
            setCheckoutErrorMsg('Stripe Code card_declined: Insufficient funds.');
            setBillingNotification('❌ Error: Payment Authorization Rejected.');
            return;
          }
          ...
          if (!updateError) {
            ...
            setBillingNotification(`🎉 Successfully upgraded to ${targetPlan.name}!`);
            onRefreshAllData();
          } else {
            throw new Error('SaaS update failed');
          }
        ...
        setCheckoutStep('failed');
        setCheckoutErrorMsg(err.message || 'Checkout failed. Please try again.');
      }
    }, 1500);
  };
                    ...
                    <div className="pt-2.5 border-t">
                      <div className="flex items-baseline">
                        <span className="text-lg font-black text-slate-900">
                          {convertAndFormatPrice(basePrice, currency)}
                        </span>
                      ...
                      <span className="text-[9px] text-indigo-605 text-indigo-600 font-extrabold block">
                        {p.priceMonthly === 0 
                          ...
                          : billingPeriod === 'yearly' 
                            ? `Billed annually (${convertAndFormatPrice(basePrice * 12, currency)}/yr)` 
                            : 'Monthly Terms'}
                      </span>
                    </div>
                ...
                <tbody className="divide-y divide-slate-100 font-medium text-secondary">
                  {invoices.map((inv) => (
                    <tr key={inv.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="p-3 font-black text-slate-900">{convertAndFormatPrice(inv.amount, currency)}</td>
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-center font-bold text-slate-705">
                <tr className="hover:bg-slate-50/40">
                  ...
                  <td className="p-3 font-mono text-slate-400">{convertAndFormatPrice(0, currency)} / mo</td>
                  <td className="p-3">{convertAndFormatPrice(19, currency)} / mo</td>
                  <td className="p-3 bg-indigo-50/35 border-x border-indigo-100 text-indigo-700 font-mono">{convertAndFormatPrice(49, currency)} / mo</td>
                  <td className="p-3">{convertAndFormatPrice(99, currency)} / mo</td>
                  <td className="p-3 font-mono text-slate-900 font-black">{convertAndFormatPrice(199, currency)} / mo</td>
                </tr>
                <tr className="hover:bg-slate-50/40">
                  ...
                  <td className="p-3 text-emerald-600 font-semibold">{convertAndFormatPrice(15, currency)}/mo ({convertAndFormatPrice(180, currency)} billed)</td>
                  <td className="p-3 bg-indigo-50/35 border-x border-indigo-100 text-indigo-700 font-bold">{convertAndFormatPrice(39, currency)}/mo ({convertAndFormatPrice(468, currency)} billed)</td>
                  <td className="p-3 text-emerald-600">{convertAndFormatPrice(79, currency)}/mo ({convertAndFormatPrice(948, currency)} billed)</td>
                  <td className="p-3 font-mono text-slate-900 font-black text-emerald-605 text-emerald-600">{convertAndFormatPrice(159, currency)}/mo ({convertAndFormatPrice(1908, currency)} billed)</td>
                </tr>
                <tr className="hover:bg-slate-50/40">
                  ...
                  <div className="bg-surface border p-3.5 rounded-xl text-xs space-y-1.5">
                    ...
                    {appliedDiscount > 0 && (
                      <div className="flex justify-between text-emerald-650 text-emerald-700 pt-1.5 border-t border-dashed">
                        ...
                        <span>-{convertAndFormatPrice(((billingPeriod === 'monthly' ? (plans.find(p => p.id === selectedPlanId)?.priceMonthly || 0) : (plans.find(p => p.id === selectedPlanId)?.priceYearly || 0) * 12) * appliedDiscount) / 100, currency)}</span>
                      </div>
                    )}
                    ...
                    <div className="flex justify-between font-black border-t pt-1.5 text-slate-900 text-sm">
                      ...
                      <span>
                        {convertAndFormatPrice((billingPeriod === 'monthly' 
                          ? (plans.find(p => p.id === selectedPlanId)?.priceMonthly || 0) 
                          : (plans.find(p => p.id === selectedPlanId)?.priceYearly || 0) * 12) * (1 - appliedDiscount / 100), currency)}
                      </span>
                    </div>
                  </div>
          ...
          <div className="bg-card rounded-2xl p-6 max-w-md w-full space-y-4 shadow-2xl border print:p-0 print:border-none print:shadow-none">
            ...
            <div className="border rounded-xl overflow-hidden text-[11px] font-medium text-secondary">
              ...
              <div className="p-3 grid grid-cols-4 text-center items-center divide-y divide-slate-100">
                ...
                <span>1</span>
                <span>{convertAndFormatPrice(viewingInvoice.amount - viewingInvoice.taxAmount, currency)}</span>
                <span className="text-right font-bold text-slate-900">{convertAndFormatPrice(viewingInvoice.amount - viewingInvoice.taxAmount, currency)}</span>
              </div>
              <div className="bg-slate-50/50 p-2.5 px-3 select-none text-right text-[10px] space-y-1 border-t">
                ...
                <p className="flex justify-between max-w-[180px] ml-auto font-black text-xs text-indigo-950 border-t pt-1 mt-1">
                  ...
                  <span>{convertAndFormatPrice(viewingInvoice.amount, currency)}</span>
                </p>
              </div>
            </div>
