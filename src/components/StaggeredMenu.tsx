...
export default function StaggeredMenu({
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    const offscreen = position === 'left' ? -100 : 100;
    const layerStates = layers.map(el => ({ el: el as HTMLElement, start: offscreen }));
  ...
  const animateText = useCallback((opening: boolean) => {
    ...
    gsap.to(inner, { yPercent: -finalShift, duration: 0.5 + lineCount * 0.07, ease: 'power4.out' });
  }, []);
  ...
  const handleItemClick = useCallback((tab: string) => {
    onNavigate?.(tab);
    closeMenu();
    ...
    >
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        <div className="sm-logo" aria-label="Logo">
          {logoUrl ? (
            ...
            <div className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-white text-[10px]" style={{ background: 'linear-gradient(135deg, #ff5f03, #ff8c42)' }}>EF</div>
          )}
        </div>
        <button
          ref={toggleBtnRef}
        ...
        >
          ...
          <span ref={iconRef} className="sm-icon" aria-hidden="true">
            <span ref={plusHRef} className="sm-icon-line sm-icon-line-h" />
      ...
      <aside id="staggered-menu-panel" ref={panelRef} className="staggered-menu-panel" aria-hidden={!open}>
          ...
          <ul className="sm-panel-list" role="list" data-numbering={displayItemNumbering || undefined}>
            {items.map((it, idx) => (
              <li className="sm-panel-itemWrap" key={it.label + idx}>
                <button
                  className={`sm-panel-item ${activeTab === it.tab ? 'sm-panel-item--active' : ''}`}
                  onClick={() => handleItemClick(it.tab)}
          ...
          {displaySocials && (
            <div className="sm-socials" aria-label="Social links">
              ...
              <ul className="sm-socials-list" role="list">
                {[{ label: 'GitHub', url: 'https://github.com/FurqanRaza4343/EstateFlow-CRM' }].map((s, i) => (
                  <li key={s.label + i}>
                    <a href={s.url} target="_blank" rel="noopener noreferrer" className="sm-socials-link">{s.label}</a>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="sm-panel-footer">
          <p style={{ color: '#6b7280', fontSize: '11px' }}>EstateFlow CRM v1.0</p>
        </div>
      </aside>
    </div>
  );
