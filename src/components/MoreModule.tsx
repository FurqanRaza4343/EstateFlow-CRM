...
interface MoreModuleProps {
  ...
  onUpdateLocalization?: (updates: {
    languagePreference: LanguageCode;
    currencyPreference: CurrencyCode;
    propertyUnitSystem: PropertySchemeType;
  }) => Promise<void>;
    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-5 h-full" id="more-module">
      ...
      <div className="md:col-span-3 bg-card p-3 border border-default rounded-2xl flex flex-col space-y-1.5 h-fit">
        ...
        <div className="p-3 bg-surface rounded-xl flex items-center justify-between text-[11px] font-bold text-secondary">
          ...
          <button
            id="more-sidebar-theme-toggle"
            onClick={onToggleTheme}
            className={`relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
              theme === 'light' ? 'bg-indigo-600' : 'bg-slate-300'
            }`}
            role="switch"
            aria-checked={theme === 'light'}
            aria-label="Toggle High Contrast Light Theme for outdoor legibility"
          >
            <span
              aria-hidden="true"
              ...
              <div className="space-y-4">
                {draftCaption && (
                  <div className="p-4 bg-slate-900 text-slate-100 rounded-2xl space-y-3 border border-slate-800 shadow-md">
                    ...
                    <AiDisclosure isDarkTheme={true} className="bg-slate-950/80 border-slate-805/60 py-1.5" />
              ...
              <div className="bg-surface p-4 border rounded-xl space-y-3">
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-0.5">
                    Toggle between the modern default Slate theme and an ultra-high-contrast Light mode designed specifically for outdoor readability under direct sunlight.
                  </p>
                </div>
                
                <div className="flex items-center justify-between border-t pt-3 text-xs">
                  <span className="font-bold text-slate-705">Outdoor High-Contrast Light Mode</span>
                  <button
