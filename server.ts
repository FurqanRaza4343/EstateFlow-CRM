...
// Auth Routes - replaced by InsForge Auth
// Legacy stubs for backward compatibility
app.post('/api/auth/register', (req, res) => {
  res.status(410).json({ error: 'Auth is now handled by InsForge. Please use the InsForge SDK.' });
});
...
// 11.5 AI Chatbot command processor helper (supports provider parameter)
app.post('/api/ai/process-command', async (req, res) => {
  const { prompt, userId, provider } = req.body;
  if (!prompt) {
...
// 13.5 WhatsApp message sending endpoint
...
// 13.7 Mistral AI direct chat endpoint
...
app.use('*', (req, res, next) => {
  ...
  if (!viteReady) {
    return res.status(503).set({ 'Content-Type': 'text/html' }).end(`<!doctype html><html><body style="display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;background:#EFEFEF;font-family:sans-serif"><div style="text-align:center"><h2 style="color:#111;font-weight:700;font-size:14px">EstateFlow CRM</h2><p style="color:#666;font-size:12px">Starting up — please wait a moment...</p></div></body></html>`);
  }
  next();
});
...
async function start() {
  ...
  viteReady = true;
  console.log('Vite ready — EstateFlow CRM fully running on port 3000');
}

start().catch(console.error);
app.listen(PORT, '0.0.0.0', () => {
  console.log('Server listening (Vite starting in background)...');
});
